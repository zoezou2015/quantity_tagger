...
public class FixSemiAddSubNetworkCompiler extends NetworkCompiler {
	...
	public FixSemiAddSubNetworkCompiler(ArrayList<String> signs) {
		this._signs = signs;
	...
	public BaseNetwork compileLabeled(int networkId, Instance inst, LocalNetworkParam param) {
		...
		int halfWindowSize = FixSemiAddSubConfig.halfWindowSize;
		...
		for (int numPos : numberPos) {
			...
			int start_pos = window.get(0);
			int end_pos = window.get(window.size() - 1);
			long b_node = this.toNode_B(numPos, start_pos, signId);
			builder.addNode(b_node);
			beforeNodes.add(b_node);
			long a_node = this.toNode_A(numPos, end_pos, signId);
			afterNodes.add(a_node);
			builder.addNode(a_node);
			builder.addEdge(a_node, new long[] { b_node });
	...
	public BaseNetwork compileUnlabeled(int networkId, Instance inst, LocalNetworkParam param) {
		...
		for (int numPos : numberPos) {
			...
			for (String s : this._signs) {
				...
				if (numberPos.size() == 3 && s.equals("zero") && !FixSemiAddSubConfig.isMath23K) {
					continue;
				}
	...
	public LatentAddSubInstance decompile(Network network) {
		...
		int size = (FixSemiAddSubConfig.halfWindowSize * 2 + 1) * numberPos.size() + 1;
