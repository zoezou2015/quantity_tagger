...
public class RelaxAddSubNetworkCompiler extends NetworkCompiler {
	...
	public RelaxAddSubNetworkCompiler(ArrayList<String> signs) {
		this._signs = signs;
	...
	public BaseNetwork compileLabeled(int networkId, Instance inst, LocalNetworkParam param) {
		...
		int halfWindowSize = RelaxAddSubConfig.halfWindowSize;
		...
		for (int numPos : numberPos) {
			...
			ArrayList<Long> numberNodes = new ArrayList<>();
			...
			for (int w : window) {
				if (w < numPos) {
					...
					long n_node = this.toNode_Num(numPos, w, signId);
					builder.addNode(n_node);
					numberNodes.add(n_node);
				} else if (w > numPos) {
			...
			allNodes.addAll(numberNodes);
			allNodes.addAll(afterNodes);
			for (int index = 0; index < allNodes.size() - 1; index++) {
				builder.addEdge(allNodes.get(index + 1), new long[] { allNodes.get(index) });
			}
			...
			for (long n_n : numberNodes) {

				// for (long a_n : afterNodes) {
				// builder.addEdge(a_n, new long[] { b_n });
				// }
				for (long c_n : children) {
					builder.addEdge(n_n, new long[] { c_n });
				}
			}

			children.clear();
			children.addAll(numberNodes);
	...
	public BaseNetwork compileUnlabeled(int networkId, Instance inst, LocalNetworkParam param) {
		...
		for (int numPos : numberPos) {
			...
			for (String s : this._signs) {
				...
				if (numberPos.size() == 3 && s.equals("zero") && !RelaxAddSubConfig.isMath23K) {
					continue;
				}

				ArrayList<Long> beforeNodes1 = new ArrayList<>();
				ArrayList<Long> afterNodes1 = new ArrayList<>();
				ArrayList<Long> numberNodes1 = new ArrayList<>();
				ArrayList<Long> allNodes1 = new ArrayList<>();
				...
				for (int w : window) {
					if (w < numPos) {
						...
						builder.addNode(b_node);
						beforeNodes.add(b_node);
						beforeNodes1.add(b_node);
					} else if (w == numPos) {
						long n_node = this.toNode_Num(numPos, w, signId);
						builder.addNode(n_node);
						numberNodes.add(n_node);
						numberNodes1.add(n_node);
					} else if (w > numPos) {
						long a_node = this.toNode_A(numPos, w, signId);
						builder.addNode(a_node);
						afterNodes.add(a_node);
						afterNodes1.add(a_node);
					}
				}
				allNodes1.addAll(beforeNodes1);
				allNodes1.addAll(numberNodes1);
				allNodes1.addAll(afterNodes1);
				for (int index = 0; index < allNodes1.size() - 1; index++) {
					builder.addEdge(allNodes1.get(index + 1), new long[] { allNodes1.get(index) });
				}
			}
	...
	public LatentAddSubInstance decompile(Network network) {
		...
		int size = (RelaxAddSubConfig.halfWindowSize * 2 + 1) * numberPos.size() + 1;
		...
		for (int i = 0; i < size; i++) {
			...
			} else if (child_arr[3] == NodeType.num.ordinal() && child_arr[4] == SubNodeType.before.ordinal()) {
				// int numPosition = numberPos.indexOf(child_arr[0]);
				// prediction.set(numPosition, this._signs.get(child_arr[2]));
				span.add(0, "B");
