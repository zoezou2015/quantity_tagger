...
public class VariantAddSubNetworkCompiler extends NetworkCompiler {
	...
	public VariantAddSubNetworkCompiler(ArrayList<String> signs) {
		this._signs = signs;
	...
	public BaseNetwork compileLabeled(int networkId, Instance inst, LocalNetworkParam param) {
		...
		for (int i = 0; i < partTextPos.size(); i++) {
			// for (int pos : partTextPos) {
			int pos = partTextPos.get(i);
		...
		for (int i = 0; i < partTextPos.size(); i++) {
			...
			if (numberPos.indexOf(pos) != -1) {
				for (String s : this._signs) {
					...
					if (numberPos.size() == 3 && s.equals("zero") && !VariantAddSubConfig.isMath23K) {
						continue;
					}
