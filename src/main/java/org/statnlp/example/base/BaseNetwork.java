...
public class BaseNetwork extends TableLookupNetwork {
	 ...
	 * Networks created through this constructor requires {@link #finalizeNetwork()}
	 * to be called before use.<br>
	 * The recommended way to create a network is through {@link NetworkBuilder}.
	 * 
	 * @param networkId
	 * @param inst
	 ...
	 * @see NetworkBuilder#build(Integer, Instance, LocalNetworkParam,
	 *      NetworkCompiler)
	 */
	...
	/**
	 * Creates a network with the node and edge arrays taken from existing ones, and
	 * the nodeCount specifies the actual number of relevant nodes.<br>
	 ...
	 * @param param
	 * @param compiler
	 * @see NetworkBuilder#quickBuild(Class, int, Instance, long[], int[][][], int,
	 *      LocalNetworkParam, NetworkCompiler)
	 */
	protected BaseNetwork(int networkId, Instance inst, long[] nodes, int[][][] children, int nodeCount,
			LocalNetworkParam param, NetworkCompiler compiler) {
	...
	/**
	 * A helper class that follows Builder pattern to ensure that the Network
	 * constructed is always complete.<br>
	 * This class is supposed to be used in a {@link NetworkCompiler}
	 * implementation.<br>
	...
	public static class NetworkBuilder<T extends BaseNetwork> {
		...
		public static NetworkBuilder<BaseNetwork> builder() {
			return new NetworkBuilder<>(BaseNetwork.class);
		}
		...
		public static <T extends BaseNetwork> NetworkBuilder<T> builder(Class<T> networkClass) {
			return new NetworkBuilder<>(networkClass);
		}

		/**
		 * Gets a network builder for the specified network class.<br>
		 * The network class provided must extend {@link BaseNetwork} and has an empty
		 * constructor.
		 * 
		 * @param networkClass
		 * @return
		 */
		private static <T extends BaseNetwork> T quickBuild(Class<T> networkClass)
		 ...
		 *            The network id
		 * @param inst
		 *            The instance associated with the network to be constructed
		 * @param nodes
		 *            The nodes that might be present in the network to be constructed.
		 * @param children
		 *            The edges that might be present in the network to be constructed.
		 * @param nodeCount
		 *            The actual number of nodes from the given nodes array that are
		 *            actually part of the network to be constructed.
		 * @param param
		 *            The LocalNetworkParam object associated with the network to be
		 *            constructed.
		 * @param compiler
		 *            The network compiler that builds the network.
		 * @return
		 */
		public static BaseNetwork quickBuild(int networkId, Instance inst, long[] nodes, int[][][] children,
				int nodeCount, LocalNetworkParam param, NetworkCompiler compiler) {
			return quickBuild(BaseNetwork.class, networkId, inst, nodes, children, nodeCount, param, compiler);
		}

		/**
		 * Quickly builds a network with the specified network class and the required
		 * parameters.<br>
		 * The network class provided must extend {@link BaseNetwork} and has a
		 * constructor with the following signature: (int networkId, Instance instance,
		 * long[] nodes, int[][][] children, int nodeCount, LocalNetworkParam param,
		 * NetworkCompiler compiler)
		 * 
		 * @param networkClass
		 *            The class of the network to be constructed
		 * @param networkId
		 *            The network id
		 * @param inst
		 *            The instance associated with the network to be constructed
		...
		public static <T extends BaseNetwork> T quickBuild(Class<T> networkClass, int networkId, Instance inst,
			...
			try {
				Constructor<T> constructor = GeneralUtils.getMatchingAvailableConstructor(networkClass, int.class,
						Instance.class, long[].class, int[][][].class, int.class, LocalNetworkParam.class,
						NetworkCompiler.class);
		...
		public NetworkBuilder(Class<T> networkClass) {
			this._children_tmp = new HashMap<>();
		 ...
		 * @return true if the node was successfully added, false if the node was
		 *         previously present.
		 */
		...
		/**
		 * Add an edge to this network. Only do this after the respective nodes are
		 * added.
		 * 
		 * @param parent
		 *            The parent node
		 * @param child
		 *            The child nodes of a SINGLE hyperedge. Note that this only add one
		 *            edge, with the parent as the root and the children as the leaves
		 *            in the hyperedge. To add multiple edges, multiple calls to this
		 *            method is necessary.
		 * @return true if the edge was added successfully, false if the edge was
		 *         previously present in the network.
		 ...
		 * This is a useful method to reduce the number of nodes and edges during
		 * network creation.
		 */
		...
		/**
		 * Finalize this network, by converting the temporary arrays for nodes and edges
		 * into the finalized one.<br>
		 * <strong>NOTE:</strong> This will create a network without any associated
		 * networkID, instance, parameters, and compiler. Use this build function if the
		 * network is not to be used in the computation directly (for example to be used
		 * as a generic unlabeled network).<br>
		 * See the {@link #build(Integer, Instance, LocalNetworkParam, NetworkCompiler)}
		 * method for creating networks suitable for computation.
		 * 
		 ...
		 * Finalize this network, by converting the temporary arrays for nodes and edges
		 * into the finalized one.<br>
		 * The supplied values, if not all null, will be passed on to the constructor of
		 * the network class.
		 * 
		 * @param networkID
		 * @param instance
		...
		public T build(Integer networkID, Instance instance, LocalNetworkParam param, NetworkCompiler compiler) {
			...
			HashMap<Long, Integer> nodesValue2IdMap = new HashMap<>();
			...
			// This is consistent with what's written in
			// http://portal.acm.org/citation.cfm?doid=1654494.1654500
			...
			if (networkID != null || instance != null || param != null || compiler != null) {
				result = quickBuild(networkClass, networkID, instance, nodeList, childrenList, nodeList.length, param,
						compiler);
			} else {
				try {
					...
					throw new RuntimeException(
							"No public empty constructor found for network class " + networkClass.getName(), e);
				}
		...
		private void checkLinkValidity(long parent, long[] children) throws NetworkException {
			for (long child : children) {
				if (child < 0) {
					// A negative child_k is not a reference to a node, it's just a number
				...
				if (child >= parent) {
					...
					throw new NetworkException("In an edge, the parent needs to have larger node ID in order to "
							+ "have a proper schedule for inference. Violation: " + parent + "\t"
		...
		private void checkNodeValidity(long node) throws NetworkException {
			if (!this._children_tmp.containsKey(node)) {
				throw new NetworkException(
						"Node not found:" + Arrays.toString(NetworkIDMapper.toHybridNodeArray(node)));
			}
		}
	}
