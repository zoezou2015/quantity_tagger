 ...
 * The instances of this class may be chained together to form a sequence of
 * feature arrays.<br>
 * The advantage of chaining becomes apparent when feature score caching (note
 * that this is different from feature caching, where the feature indices
 * themselves are cached) is used, since if only the weights of some features
 * change, only those feature arrays with changes will be recalculated.<br>
 * In the implementation, the feature indices are stored inside another class:
 * {@link FeatureBox}.
 */
public class FeatureArray implements Serializable {
	...
	/**
	 * The total score of this feature array, including all other feature arrays
	 * which are chained after this.
	 */
	private double _totalScore;
	/**
	 * The internal feature box containing the feature indices and the cached local
	 * score.
	 */
	private FeatureBox _fb;
	/**
	 * The flag signifying the scope of the feature indices, whether local (per
	 * thread) or global (combined feature map).
	 */
	...
	/**
	 * A feature with very negative score, used to signify that the hyperpath
	 * containing this feature should not be selected
	 */
	...
	// FeatureArray(-10000000);
	private FeatureArray _next;
	protected int[] dstNodes;

	/**
	 * Merges the features in <code>fs</code> and in <code>next</code><br>
	 * <strong>IMPORTANT NOTE:</strong> to use caching, please use
	 * {@link FeatureManager#createFeatureArray(Network, int[], FeatureArray)}
	 * instead, combined with setting {@link NetworkConfig#AVOID_DUPLICATE_FEATURES}
	 * to true.
	 * 
	 * @param fs
	 * @param next
	...
	/**
	 * Construct a feature array containing the features identified by their
	 * indices<br>
	 * <strong>IMPORTANT NOTE:</strong> to use caching, please use
	 * {@link FeatureManager#createFeatureArray(Network, int[])} instead, combined
	 * with setting {@link NetworkConfig#AVOID_DUPLICATE_FEATURES} to true.
	 * 
	 * @param fs
	 */
	...
	/**
	 * Creates a new FeatureArray based on the feature indices in the given
	 * FeatureBox object.<br>
	 * If you do not have FeatureBox object, perhaps you might want to check
	 * {@link #FeatureArray(int[])}.
	 * 
	 * @param fb
	 ...
	 * Usually if the version (i.e., weights) is same, the score won't be calculated
	 * again.<br>
	 * But in mean field inference, the joint feature value often changes which
	 * makes the score change although the version (i.e., weights) doesn't change.
	 * 
	 * @param alwaysChange
	 */
	...
	/**
	 * Call this for those joint feature (array) in mean field inference. <br>
	 * It specify the destination node (the current node joint to) in the other
	 * structure.
	 * 
	 * @param dstNodes:
	 *            for each feature index, it will joint to one node. The size should
	 *            be same as feature box size.
	 */
	...
	public FeatureArray toLocal(LocalNetworkParam param) {
		...
		FeatureArray fa;
		if (this._next != null) {
			fa = new FeatureArray(FeatureBox.getFeatureBox(fs_local, fv_local, param), this._next.toLocal(param)); // saving
																													// memory
		} else {
	...
	/**
	 * Returns the list of feature indices contained in this feature array. Note
	 * that this excludes the feature indices contained in the chained feature
	 * arrays.
	 * 
	 * @return
	 */
	 ...
	 * 
	 * @param next
	 *            The next feature array in this chain.
	 */
	...
	public double getScore(LocalNetworkParam param, int version) {
		...
		if (this._fb._version != version) {
			this._fb._currScore = NetworkConfig.USE_FEATURE_VALUE
					? this.computeScore(param, this.getCurrent(), this._fb._fv)
	 ...
	 * @param param
	 * @param <featureIdx,
	 *            targetNode> map, the target node is the corresponding node.
	 * @param marginals
	 *            score array, serve as being the feature value.
	 * @return
	 */
	...
	/**
	 * Returns the number of elements in the feature array, including all the
	 * subsequent feature arrays in the chain.
	 * 
	 * @return
	 */
	public int size() {
