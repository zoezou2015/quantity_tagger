...
public class NetworkConfig {
	...
	public static enum ModelStatus {
		TRAINING, DEV_IN_TRAINING, TESTING;
	}
	...
	public static enum StoppingCriteria {
		MAX_ITERATION_REACHED, SMALL_ABSOLUTE_CHANGE, SMALL_RELATIVE_CHANGE,;
	}
	...
	/**
	 * Enable to try to save memory by caching feature arrays to avoid duplicate
	 * feature arrays to be stored in memory.<br>
	 * Note that the amount of memory-saving depends on how the FeatureArrays are
	 * defined.<br>
	 * If there are lots of repeating feature arrays with the exact same sequence of
	 * feature indices, then enabling this might be beneficial, but otherwise, it
	 * will actually increase memory usage and time.<br>
	 * If you are using this, it's best to split feature arrays into multiple
	 * arrays, and then chain them together using the "next" mechanism in
	 * FeatureArray.<br>
	 * See {@link FeatureArray#FeatureArray(int[], FeatureArray)} for more
	 * information.
	 */
	...
	/**
	 * Limit the size of the priority queue used in {@link Hypothesis} class when
	 * decoding.<br>
	 * Setting this to 0 will remove the limit, making the data structure slightly
	 * faster.<br>
	 * The size limit of the priority queue will also affect the highest supported k
	 * in top-k decoding, with <br>
	 */
	public static int PRIORITY_QUEUE_SIZE_LIMIT = 0;

	/***
	 * Neural network related flags. Please read carefully about the README.txt to
	 * install the NN server and also the communication package for Neural CRF
	 */
	...
	/**
	 * If true: Optimized the neural net in CRF. optimizer in neural config must be
	 * set to none
	 **/
	public static boolean OPTIMIZE_NEURAL = true; // false means not update the neural network parameters in CRF. false
													// is faster
	...
	/**
	 * Initialize the feature value provider weights in java It will override the
	 * embedding weights if you have.
	 */
	 ...
	 */
	/**
	 * The number of internal mean-field updates to be done in between each normal
	 * iterations.
	 */
	...
	/**
	 * Currently only used by Mean-field inference. That's why protected. true if
	 * mean-field, false otherwise
	 */
