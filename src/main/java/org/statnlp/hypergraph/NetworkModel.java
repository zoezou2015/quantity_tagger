...
public abstract class NetworkModel implements Serializable {
	...
	public Instance[][] splitInstancesForTest(Instance[] testInsts) {
		...
		ArrayList<ArrayList<Instance>> insts_list = new ArrayList<>();
		int threadId;
	 ...
	 * 
	 * @param clazz
	 *            The class of the viewer engine to be used to visualize the
	 *            instances.
	 * @throws InterruptedException
	 *             If there are interruptions during the compilation process of the
	 *             instances in multi-threaded setting.
	 * @throws IllegalStateException
	 *             If train method has not been called. This means there are no
	 ...
	 *             If the viewer engine specified does not implement the correct
	 *             constructor. The viewer engine needs to implement the constructor
	 *             with signature (NetworkCompiler, FeatureManager)
	 * @see {@link #visualize(Class, Instance[])}
	 * @see {@link #visualize(Class, Instance[], int)}
	 */
	public void visualize(Class<? extends VisualizationViewerEngine> clazz)
			throws InterruptedException, IllegalArgumentException, IllegalStateException {
		if (unlabeledNetworkByInstanceId == null) {
			throw new IllegalStateException(
					"No previously used instances found. Please specify the instances to be visualized.");
		}
	 ...
	 *            instances.
	 * @param allInstances
	 *            The instances to be visualized
	 * @param numInstances
	 *            The number of instances to be visualized. This can be less than
	 *            the number of instances in allInstances
	 * @throws InterruptedException
	 *             If there are interruptions during the compilation process of the
	 *             instances in multi-threaded setting.
	 * @throws IllegalArgumentException
	 ...
	 *             with signature (NetworkCompiler, FeatureManager)
	 */
	public void visualize(Class<? extends VisualizationViewerEngine> clazz, Instance[] allInstances)
		...
		try {
			...
			throw new IllegalArgumentException("The viewer class " + clazz.getName()
					+ " must implement the constructor with signature (InstanceParser), or pass in an instantiated viewer.");
		}
	}
	 ...
	 * 
	 * @param viewer
	 *            The viewer engine to be used to visualize the instances.
	 * @param allInstances
	 *            The instances to be visualized
	 * @param numInstances
	 *            The number of instances to be visualized. This can be less than
	...
	public void train(Instance[] trainInsts, int maxNumIterations, String modelPrefix) throws InterruptedException {
		train(trainInsts, maxNumIterations, null, null, -1, modelPrefix);
	}

	public void train(Instance[] trainInsts, int maxNumIterations, Instance[] devInstances,
			Function<Instance[], Metric> evalFunction, int k) throws InterruptedException {
		train(trainInsts, maxNumIterations, devInstances, evalFunction, k, "");
	}

	/**
	 * Starts training using the instances given for up to specified number of
	 * iterations.
	 * 
	 * @param trainInstances
	 *            The instances on which this model should be trained.
	 * @param maxNumIterations
	 *            The maximum number of iterations the training should go.
	 * @param devInstance
	 *            development set for evaluation
	 * @param evalFunction
	 *            the evaluation function used to evaluate the development set
	 * @param k
	 *            The specified number used to evaluate the development set in every
	 *            <tt>k</tt> iterations.
	 * @throws InterruptedException
	 *             If there are interruptions during multi-threaded training.
	 */
	public void train(Instance[] trainInstances, int maxNumIterations, Instance[] devInstances,
			Function<Instance[], Metric> evalFunction, int k, String modelPrefix) throws InterruptedException {
		...
		/*
		 * Pre-compile the networks In mean-field, we need to pre-compile because we
		 * need the unlabeled network information in feature extraction process for the
		 * labeled network.
		 */
		...
		for (int threadId = 0; threadId < this._numThreads; threadId++) {
			if (NetworkConfig.BUILD_FEATURES_FROM_LABELED_ONLY) {
				// We extract features only from labeled instance in the first touch, so we
				// don't know what
				// features are present in each thread. So copy all features to each thread.
				// Since we extract only from labeled instances, the feature index will be
				// smaller
				this._fm.addIntoLocalFeatures(
						this._learners[threadId].getLocalNetworkParam()._globalFeature2LocalFeature);
			}
		...
		if (NetworkConfig.CACHE_FEATURES_DURING_TRAINING) {
			for (int threadId = 0; threadId < this._numThreads; threadId++) {
				// This was previously in each LocalNetworkLearnerThread finalizeIt, but moved
				// here since
				// when we call finalizeIt above, it should not delete this variable first,
				// because we were
		...
		try {
			...
			for (int it = 0; it <= maxNumIterations; it++) {
				if (NetworkConfig.USE_BATCH_TRAINING) {
					batchInstIds.clear();
					// at each epoch, shuffle the inst ids. and reset the set, which is already in
					// the learner thread
				...
				for (LocalNetworkLearnerThread learner : this._learners) {
					...
					if (NetworkConfig.USE_BATCH_TRAINING)
						learner.setInstanceIdSet(batchInstIds);
					else
						learner.setTrainInstanceIdSet(new TIntHashSet(instIds));
				}
				...
				epochObj += obj;
				if (!NetworkConfig.USE_BATCH_TRAINING) {
					print(String.format("Iteration %d: Obj=%-18.12f Time=%.3fs %.12f Total time: %.3fs", it,
							multiplier * obj, time / 1.0e9, obj / obj_old, (System.nanoTime() - startTime) / 1.0e9),
							outstreams);
				} else {
				...
				if (devInstances != null && evalFunction != null && k > 0 && (it + 1) % k == 0) {
					/**
					 * Evaluate on the development set after k iteration of parameter updates This
					 * has to be after update (i.e. now) if using neural as the dev output will
					 * modify the size of outputTensor which in turn modify the countOutputTensor
					 */
				...
				if (offset >= instIds.size()) {
					...
					print(String.format("Epoch %d: Obj=%-18.12f Time=%.3fs Total time: %.3fs", epochNum++,
							multiplier * epochObj * instIds.size() / (size + offset), (time - epochStartTime) / 1.0e9,
							(time - startTime) / 1.0e9), outstreams);
					batchId = 0;
					offset = 0;
					epochObj = 0.0;
					epochStartTime = System.nanoTime();
				}
				if (NetworkConfig.TRAIN_MODE_IS_GENERATIVE && it > 1 && obj < obj_old
						&& Math.abs(obj - obj_old) > 1E-5) {
				...
				if (lastIter) {
					print("Training completes. The specified number of iterations (" + it + ") has passed.",
							outstreams);
					break;
				}
				if (done) {
					print("Training completes. No significant progress (<objtol) after " + it + " iterations.",
							outstreams);
					break;
				}
			}
	 ...
	 * @throws InterruptedException
	 */
	private void evaluateDevelopment(Instance[] devInsts, Function<Instance[], Metric> evalFunction)
			throws InterruptedException {
	...
	private void preCompileNetworks(Instance[][] insts) throws InterruptedException {
		for (int threadId = 0; threadId < this._numThreads; threadId++) {
			this._learners[threadId] = new LocalNetworkLearnerThread(threadId, this._fm, insts[threadId],
					this._compiler, -1);
	...
	private void touch(Instance[][] insts, boolean keepExisting) throws InterruptedException {
		if (!NetworkConfig.PARALLEL_FEATURE_EXTRACTION || NetworkConfig.NUM_THREADS == 1) {
			...
			if (NetworkConfig.PRE_COMPILE_NETWORKS || !keepExisting) {
				// this one is because in the first touch, we don't have the exisiting threads
				// if not precompile network.
				// So we merge in first feature extraction.
				// If precompile network, we keep exisiting threads. But we still need to merge
				// features because
	...
	/**
	 * Tests the instances based on the learned parameters, caching the features
	 * extracted.<br>
	 * The caching is useful if one needs to decode the instances multiple times
	 * after changing some of the parameters (e.g., for tuning).<br>
	 * The predictions can be obtained through {@link Instance#getPrediction()}.
	 * 
	 * @param instances
	 *            The instances to be tested.
	 * @param cacheFeatures
	 *            Whether to cache the features from the instances.
	 * @return The same instance array, with the {@code prediction} field assigned.
	 * @throws InterruptedException
	 */
	...
	/**
	 * Tests the instances based on the learned parameters, taking the top-k
	 * structures.<br>
	 * The best predictions can be obtained through
	 * {@link Instance#getPrediction()}, while the top-k predictions can be obtained
	 * through {@link Instance#getTopKPredictions()}.
	 * 
	 * @param instances
	 *            The instances to be tested.
	 * @param numPredictionsGenerated
	 *            The number of top-k structures to be decoded.
	 * @return The same instance array, with the {@code prediction} field assigned.
	 * @throws InterruptedException
	 */
	...
	/**
	 * Tests the instances based on the learned parameters, taking the top-k
	 * structures, caching the features extracted.<br>
	 * The caching is useful if one needs to decode the instances multiple times
	 * after changing some of the parameters (e.g., for tuning).<br>
	 * The best predictions can be obtained through
	 * {@link Instance#getPrediction()}, while the top-k predictions can be obtained
	 ...
	 * @throws InterruptedException
	 */
	public Instance[] test(Instance[] instances, boolean cacheFeatures, int numPredictionsGenerated)
			throws InterruptedException {
	...
	/**
	 * Decodes the instances based on the learned parameters, caching the features
	 * extracted.<br>
	 * The caching is useful if one needs to decode the instances multiple times
	 * after changing some of the parameters (e.g., for tuning).<br>
	 * The predictions can be obtained through {@link Instance#getPrediction()}.
	...
	/**
	 * Decodes the instances based on the learned parameters, taking the top-k
	 * structures.<br>
	 * The best predictions can be obtained through
	 * {@link Instance#getPrediction()}, while the top-k predictions can be obtained
	 * through {@link Instance#getTopKPredictions()}.
	 ...
	 * @throws InterruptedException
	 */
	public Instance[] decode(Instance[] instances, boolean cacheFeatures, int numPredictionsGenerated)
			throws InterruptedException {
		...
		for (int threadId = 0; threadId < this._numThreads; threadId++) {
			if (cacheFeatures || NetworkConfig.FEATURE_TOUCH_TEST) {
				if (this._decoders[threadId] != null) {
					this._decoders[threadId] = new LocalNetworkDecoderThread(threadId, this._fm, insts[threadId],
							this._compiler, this._decoders[threadId].getParam(), true, numPredictionsGenerated);
				} else {
					this._decoders[threadId] = new LocalNetworkDecoderThread(threadId, this._fm, insts[threadId],
							this._compiler, true, numPredictionsGenerated);
				}
			} else {
				this._decoders[threadId] = new LocalNetworkDecoderThread(threadId, this._fm, insts[threadId],
						this._compiler, false, numPredictionsGenerated);
			}
		}
		...
		if (NetworkConfig.USE_NEURAL_FEATURES) {
			...
			// Important for batch training, since we don't use batch in decoding (check
			// LocalNetworkParam, add hyperedge).
