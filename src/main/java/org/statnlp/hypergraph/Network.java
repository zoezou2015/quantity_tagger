...
public abstract class Network implements Serializable, HyperGraph {
	...
	/**
	 * The working array for each thread for calculating inside scores This is done
	 * to avoid reallocating a new array for each network
	 */
	protected static double[][] insideSharedArray = new double[NetworkConfig.NUM_THREADS][]; // TODO: The value of
																								...
																								// Network class
	/**
	 * The working array for each thread for calculating outside scores This is done
	 * to avoid reallocating a new array for each network
	 */
	protected static double[][] outsideSharedArray = new double[NetworkConfig.NUM_THREADS][];

	/**
	 * The working array for each thread for calculating max scores This is done to
	 * avoid reallocating a new array for each network
	 */
	...
	/**
	 * store the information of structure of each node in network. the value
	 * (structure) of each node is specified by user. Currently used for mean-field
	 * inference
	 **/
	...
	protected NodeHypothesis[] getHypothesisSharedArray() {
		if (hypothesisSharedArray[this._threadId] == null
				|| this.countNodes() > hypothesisSharedArray[this._threadId].length) {
	...
	protected double[] getMarginalSharedArray() {
		if (unlabeledMarginalSharedArray[this._threadId] == null
				|| this.countNodes() > unlabeledMarginalSharedArray[this._threadId].length)
	...
	protected double[] getNewMarginalSharedArray() {
		if (unlabeledNewMarginalSharedArray[this._threadId] == null
				|| this.countNodes() > unlabeledNewMarginalSharedArray[this._threadId].length)
	...
	/**
	 * Return the max path according to the configuration specified in the bestPath
	 * IndexedScore object.<br>
	...
	/**
	 * Calculate the marginal score for all nodes this marginal is used for
	...
	public void marginal() {
		...
		for (int k = 0; k < this.countNodes(); k++) {
			...
			if (NetworkConfig.MAX_MARGINAL_DECODING || (NetworkConfig.INFERENCE == InferenceType.MEAN_FIELD
					&& !this.getInstance().isLabeled() && !this.isRemoved(k))) {
				this.marginal(k);
	...
	protected void inside(int k) {
...

		{
			...
			if (ignoreflag) {
				...
				double score = NetworkConfig.INFERENCE == InferenceType.MEAN_FIELD ? fa.getScore_MF_Version(this._param,
						this.getUnlabeledNetwork().getMarginalSharedArray(), globalParamVersion)
				...
				if (NetworkConfig.USE_NEURAL_FEATURES) {
					score += this._param._fm.getParam_G().getNNParamG().getNNScore(this, k, children_k,
							children_k_index);
				}
...

			{
				...
				if (ignoreflag) {
					...
					double score = NetworkConfig.INFERENCE == InferenceType.MEAN_FIELD
							? fa.getScore_MF_Version(this._param, this.getUnlabeledNetwork().getMarginalSharedArray(),
									globalParamVersion)
							: fa.getScore(this._param, globalParamVersion);
					if (NetworkConfig.MODEL_TYPE.USE_COST) {
						try {
	 ...
	 * 
	 * Not really abstract methods here since other projects do not implement due to
	 * old version.
	 **/
	/**
	 * Only required when we used the mean-field inference method. Need to
	 * implemented in user's own network. No need to implement if not using
	 * mean-field inference.
	 */
	...
	/**
	 * specifically for mean-field experiments Need to implement this while using
	 * mean field inference
	 */
	...
	/**
	 * Compare the new and old marginal map decide to continue mean-field update or
	 * not
	 * 
