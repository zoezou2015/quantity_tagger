...
public class GlobalNetworkParam implements Serializable {
	...
	public GlobalNetworkParam(OptimizerFactory optimizerFactory, GlobalNeuralNetworkParam nn_param_g) {
		...
		this._featureIntMap = new TIntObjectHashMap<>();
		if (NetworkConfig.TRAIN_MODE_IS_GENERATIVE) {
			this._type2inputMap = new TIntObjectHashMap<>();
		}
	 ...
	 * allowed.
	 * 
	 * @param keepExistingWeights
	 *            Whether to keep existing weights
	 */
	public void lockIt(boolean keepExistingWeights) {
		...
		/**
		 * Must prepare the feature map before reset counts and obj The reset will use
		 * feature2rep.
		 **/
		if (this._storeFeatureReps) {
	 ...
	 * @param input
	 * @return
	 * @deprecated Please use {@link #toFeature(Network, String, String, String)}
	 *             instead.
	 */
	@Deprecated
	...
	/**
	 * Returns the feature ID of the specified feature from the global feature
	 * index.<br>
	...
	/**
	 * Returns the feature ID of the specified feature from the specified feature
	 * index.<br>
	 * If the feature is not present in the feature index, return -1.
	 * 
	 * @param type
		...
		if (this._opt.name().contains("Gradient Descent Optimizer")) {
			...
			if (NetworkConfig.USE_NEURAL_FEATURES) {
				...
				for (AbstractNeuralNetwork net : this._nn_param_g.getAllNets()) {
					...
					if (params == null || gradParams == null)
						continue;
