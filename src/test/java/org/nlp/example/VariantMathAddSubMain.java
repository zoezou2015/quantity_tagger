...
import org.statnlp.example.math_add_sub_variant.VariantAddSubConfig;
import org.statnlp.example.math_add_sub_variant.VariantAddSubFeatureManager;
import org.statnlp.example.math_add_sub_variant.VariantAddSubNetworkCompiler;
import org.statnlp.example.math_add_sub_variant.VariantAddSubReader;
import org.statnlp.hypergraph.DiscriminativeNetworkModel;
import org.statnlp.hypergraph.GlobalNetworkParam;
import org.statnlp.hypergraph.NetworkConfig;
import org.statnlp.hypergraph.NetworkModel;

public class VariantMathAddSubMain {
	...
	private static void argsParser(String[] args) {
		...
		while (i < args.length) {
			if (args[i].equals("-iter")) {
				...
				VariantAddSubConfig.halfWindowSize = Integer.parseInt(args[++i]) - 1;
			} else if (args[i].equals("-lang")) {
				String lang = args[++i];
				if (lang.equals("en")) {
					VariantAddSubConfig.isMath23K = false;
					VariantAddSubConfig.lang = "en";
				} else if (lang.equals("zh")) {
					VariantAddSubConfig.isMath23K = true;
					VariantAddSubConfig.lang = "zh";
				} else {
	...
	public static void main(String[] args) throws IOException, JSONException, InterruptedException {
		...
		if (VariantAddSubConfig.isMath23K) {
			esfc = new ExtractSentenceFeaturesChinese();
			VariantAddSubConfig.cv_fold = 3;
			file = "data/math/Math23K/Math23K_AddSub.json";
			numIterations = 250;
			VariantAddSubConfig.lang = "zh";
			VariantAddSubConfig.halfWindowSize = 3;
		} else {
			esf = new ExtractSentenceFeatures();
			VariantAddSubConfig.lang = "en";
			VariantAddSubConfig.halfWindowSize = 1;
		}
		if (debug) {
		...
		for (int fold = 0; fold < VariantAddSubConfig.cv_fold; fold++) {
			...
			if (!VariantAddSubConfig.isMath23K) {
				...
				if (fold == 0) {
					train_output_file = "data/math/AddSub/Variant_MA1_train_output.txt";
					test_output_file = "data/math/AddSub/Variant_MA1_test_output.txt";
				} else if (fold == 1) {
					train_output_file = "data/math/AddSub/Variant_MA2_train_output.txt";
					test_output_file = "data/math/AddSub/Variant_MA2_test_output.txt";
				} else if (fold == 2) {
					train_output_file = "data/math/AddSub/Variant_IXL_train_output.txt";
					test_output_file = "data/math/AddSub/Variant_IXL_test_output.txt";
				}
				...
				if (validation) {
					...
					if (fold == 0) {
						train_output_file = "data/math/AddSub/dev_Variant_MA1_train_output.txt";
						test_output_file = "data/math/AddSub/dev_Variant_MA1_test_output.txt";
					} else if (fold == 1) {
						train_output_file = "data/math/AddSub/dev_Variant_MA2_train_output.txt";
						test_output_file = "data/math/AddSub/dev_Variant_MA2_test_output.txt";
					} else if (fold == 2) {
						train_output_file = "data/math/AddSub/dev_Variant_IXL_train_output.txt";
						test_output_file = "data/math/AddSub/dev_Variant_IXL_test_output.txt";
					}
				...
				train_insts = VariantAddSubReader.InstanceReader(esf, file, train_ids, true);
				test_insts = VariantAddSubReader.InstanceReader(esf, file, test_ids, false);
			} else {
				train_ids = "data/math/Math23K/" + VariantAddSubConfig.cv_fold + "-fold/fold-" + fold + "/train.txt";
				test_ids = "data/math/Math23K/" + VariantAddSubConfig.cv_fold + "-fold/fold-" + fold + "/test.txt";

				train_output_file = "data/math/Math23K/" + VariantAddSubConfig.cv_fold + "-fold/fold-" + fold
						+ "/variant_train_out.txt";
				test_output_file = "data/math/Math23K/" + VariantAddSubConfig.cv_fold + "-fold/fold-" + fold
						+ "/variant_test_out.txt";
				...
				test_savePath = "data/math/Math23K/" + AddSubConfig.cv_fold + "-fold/dev_variant_test_data" + fold
						+ ".ser";
				if (validation) {
					train_ids = "data/math/Math23K/" + VariantAddSubConfig.cv_fold + "-fold/fold-" + fold
							+ "/dev_train.txt";
					test_ids = "data/math/Math23K/" + VariantAddSubConfig.cv_fold + "-fold/fold-" + fold
							+ "/dev_test.txt";

					train_output_file = "data/math/Math23K/" + VariantAddSubConfig.cv_fold + "-fold/fold-" + fold
							+ "/dev_variant_train_out.txt";
					test_output_file = "data/math/Math23K/" + VariantAddSubConfig.cv_fold + "-fold/fold-" + fold
							+ "/dev_variant_test_out.txt";
					train_savePath = "data/math/Math23K/" + AddSubConfig.cv_fold + "-fold/dev_variant_train_data" + fold
							+ ".ser";
					test_savePath = "data/math/Math23K/" + AddSubConfig.cv_fold + "-fold/dev_variant_test_data" + fold
							+ ".ser";
				...
				train_insts = VariantAddSubReader.InstanceReader_Math23K(esfc, file, train_ids, train_savePath, true);
				test_insts = VariantAddSubReader.InstanceReader_Math23K(esfc, file, test_ids, test_savePath, false);
			}
			...
			VariantAddSubNetworkCompiler compiler = new VariantAddSubNetworkCompiler(signs);
			VariantAddSubFeatureManager fm = new VariantAddSubFeatureManager(gnp);
