...
import org.statnlp.example.math_add_sub.AddSubFeatureManager;
import org.statnlp.example.math_add_sub.AddSubInstance;
import org.statnlp.example.math_add_sub.AddSubNetworkCompiler;
...
public class MathAddSubMain {
	...
	public static void main(String[] args) throws IOException, JSONException, InterruptedException {
		...
		ExtractSentenceFeatures esf_en = null;
		if (AddSubConfig.isMath23K) {
			...
			esf_en = new ExtractSentenceFeatures();
		}
		if (debug) {
		...
		for (int fold = 0; fold < AddSubConfig.cv_fold; fold++) {
			...
			if (!AddSubConfig.isMath23K) {
				...
				train_insts = AddSubReader.InstanceReader(esf_en, file, train_ids, true);
				// // train_insts_clone = AddSubReader.InstanceReader(file, train_ids, false);
				test_insts = AddSubReader.InstanceReader(esf_en, file, test_ids, false);
			...
			AddSubNetworkCompiler compiler = new AddSubNetworkCompiler(signs);
			AddSubFeatureManager fm = new AddSubFeatureManager(gnp);
